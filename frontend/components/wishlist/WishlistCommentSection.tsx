...
} from "@/lib/api/wishlistComments";
import { uploadImage } from "@/lib/api/upload";
import { formatRelativeTimeVN } from "@/lib/utils";
import type { PostComment } from "@/types/comment";

interface WishlistCommentSectionProps {
  wishlistId: string;
...
export function WishlistCommentSection({
  wishlistId,
  initialComments,
}: WishlistCommentSectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await createWishlistComment(
        wishlistId,
        optimistic.content,
        uploadedUrls,
      );
  ...
  const handleDelete = async (commentId: string) => {
    ...
    try {
      await deleteWishlistComment(wishlistId, commentId);
    } catch (err: any) {
  ...
  const canSubmit =
    !submitting && (content.trim().length > 0 || pendingFiles.length > 0);
