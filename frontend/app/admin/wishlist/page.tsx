...
import { Pin, ExternalLink, Heart } from "lucide-react";
import { Badge } from "@/components/ui/Badge";
import { Avatar } from "@/components/ui/Avatar";
import { SectionHeading } from "@/components/ui/SectionHeading";
import { AdminWishlistFilters } from "@/components/admin/AdminWishlistFilters";
import { AdminWishlistActions } from "@/components/admin/AdminWishlistActions";
import { listAdminWishlistServer } from "@/lib/api/adminWishlist.server";
...
export default async function AdminWishlistPage({
  searchParams,
}: AdminWishlistPageProps) {
  const params = await searchParams;
  const page = Math.max(1, parseInt(params.page ?? "1", 10) || 1);
  const limit = 20;

  let items: WishlistPost[] = [];
  let total = 0;
  try {
    const result = await listAdminWishlistServer({
    ...
    console.error("[admin/wishlist] listAdminWishlistServer failed", error);
  }
  ...
  const buildPageHref = (p: number) => {
    ...
    return `/admin/wishlist${sp.size > 0 ? `?${sp}` : ""}`;
  };

  return (
    <div className="p-8">
      <SectionHeading>Quản lý Wishlist</SectionHeading>
      <p className="mt-1 text-sm text-muted-foreground">{total} wishlist</p>
        ...
        {items.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[var(--border-green)]">
              {items.map((item) => {
                const author =
                  typeof item.author === "string" ? null : item.author;
                const firstImage = item.images?.[0] ?? null;
                return (
                  <tr key={item._id} className="hover:bg-bg-cream/50">
                    {/* Thumbnail */}
                    ...
                    <td className="px-4 py-3">
                      ...
                      <p className="mt-0.5 text-xs text-muted-foreground">
                        SL: {item.quantity} · ♥ {item.likesCount}
                      </p>
                    </td>
                    ...
                    <td className="hidden px-4 py-3 lg:table-cell">
                      <Badge variant={item.status === "open" ? "success" : "default"}>
                        {item.status === "open" ? "Đang mở" : "Đã đáp ứng"}
                      </Badge>
                    </td>
